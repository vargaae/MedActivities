...
function focusDialogOnEnter(container: HTMLElement) {
  // Focus during the enter lifecycle, before ModalManager hides the page.
  // MUI retains its focus trap; restoration waits for the exit transition.
  container
...
export default function PractitionersPage() {
  ...
  const dialogFocusProps = {
    // MUI's default restore runs while the background is still aria-hidden.
    disableRestoreFocus: true,
    slotProps: {
      transition: {
        onEnter: focusDialogOnEnter,
        onExited: () => {
  ...
  const [remove, setRemove] = useState<Doctor | null>(null);
  ...
  const list = useQuery({
    ...
    queryFn: async ({ signal }) =>
      (await agent.get<Doctor[]>("/practitioners", { signal })).data,
  });
  const users = useQuery({
    queryKey: ["practitioner-accounts", session.version],
    enabled: !!admin,
    queryFn: async ({ signal }) =>
      (
  ...
  const schedule = useQuery({
    queryKey: ["working-hours", selected?.id],
    enabled: !!selected,
    queryFn: async ({ signal }) =>
      (
        await agent.get<Hours[]>(
          `/practitioners/${selected!.id}/working-hours`,
          { signal },
        )
      ).data,
  });
  const canSeeEvents =
    ...
    selected?.id === session.data?.ownPractitioner?.id;
  const events = useQuery({
    queryKey: ["doctor-events", session.version, selected?.id],
    enabled: !!selected && !!canSeeEvents,
    queryFn: async ({ signal }) =>
      (
        await agent.get<{ id: string; title: string; date: string }[]>(
          `/practitioners/${selected!.id}/activities`,
          { signal },
        )
      ).data,
  });
    ...
    <Box sx={{ display: "grid", gap: 2 }}>
      ...
      {list.data
        ...
        .map((d) => (
          <Paper key={d.id} sx={{ p: 2 }}>
            ...
            <Typography>
              Időpontfoglalás:{" "}
              {d.bookingEnabled ? "engedélyezett" : "kikapcsolva"}
            </Typography>
            <Button
              onClick={(event) => {
                dialogTrigger.current = event.currentTarget;
              ...
              <>
                <Button
                  ...
                  onClick={(event) => {
                    ...
                    void run(async () => {
                      setForm(
                        (await agent.get<Doctor>(`/practitioners/${d.id}`))
                          .data,
                      );
                    });
                  }}
      ...
      >
        ...
        <DialogContent>
          ...
          <Box
            ...
            onSubmit={(e) => {
              ...
              void run(async () => {
                if (form?.id)
                  await agent.put(`/practitioners/${form.id}`, form);
                else await agent.post("/practitioners", form);
                setForm(null);
              });
            }}
          >
            <TextField
              ...
              name="name"
              label="Teljes név"
              slotProps={{ htmlInput: { "data-dialog-initial-focus": true } }}
              required
              {...field("name")}
            />
            <TextField
            ...
            {!form?.id && users.data?.length === 0 && (
              <Alert severity="info">
                ...
                Practitioner-fiókot. A regisztráció már automatikusan létrehozza
                a kezelőprofilt: azt a listában szerkesztheted.
              </Alert>
            )}
            <TextField
              label="Szakterület (orvos / gyógytornász)"
              required
              {...field("specialty")}
            />
            <TextField label="Város" required {...field("city")} />
            <TextField
        ...
        <DialogActions>
          ...
          <Button type="submit" form="doctor-form" disabled={busy}>
            Mentés
          </Button>
        </DialogActions>
      </Dialog>
      <Dialog
        ...
        maxWidth="md"
      >
        <DialogTitle tabIndex={-1} data-dialog-initial-focus>
          {selected?.name}
        </DialogTitle>
        <DialogContent>
          {error && <Alert severity="error">{error}</Alert>}
          {admin && (
            <Button
              ...
              onClick={() =>
                void run(async () => {
                  const enabled = !selected!.bookingEnabled;
                  await agent.put(
                    `/practitioners/${selected!.id}/booking-enabled`,
                    { bookingEnabled: enabled },
                  );
                  setSelected({ ...selected!, bookingEnabled: enabled });
                })
              }
            >
              {selected?.bookingEnabled
                ? "Foglalás kikapcsolása"
                : "Foglalás engedélyezése"}
            </Button>
          )}
          <Typography variant="h6">Heti munkaidő</Typography>
          {schedule.data?.map((w) => (
            <Typography key={w.dayOfWeek}>
              {days[w.dayOfWeek]}:{" "}
              {w.isWorkingDay ? `${w.startTime}–${w.endTime}` : "Szabadnap"}
            </Typography>
          ))}
          {schedule.isError && (
            <Alert severity="error">A munkaidő nem tölthető be.</Alert>
          )}
          {admin && (
            <Box sx={{ display: "grid", gap: 2, my: 2 }}>
              <TextField
                ...
                onChange={(e) => {
                  ...
                  setHours(
                    schedule.data?.find((w) => w.dayOfWeek === dayOfWeek) ?? {
                      dayOfWeek,
                      isWorkingDay: true,
                      startTime: "08:00",
                      endTime: "16:00",
              ...
              <TextField
                select
                label="Munkanap"
                value={String(hours.isWorkingDay)}
                onChange={(e) =>
                  setHours({
                    ...hours,
              ...
              >
                <MenuItem value="true">Munkanap</MenuItem>
                <MenuItem value="false">Szabadnap</MenuItem>
              </TextField>
              <TextField
                label="Kezdés"
                type="time"
                  ...
                  void run(async () => {
                    await agent.put(
                      `/practitioners/${selected!.id}/working-hours`,
                      {
                        ...hours,
                        startTime:
                          hours.startTime.length === 5
                            ? hours.startTime + ":00"
                            : hours.startTime,
                        endTime:
                          hours.endTime.length === 5
                            ? hours.endTime + ":00"
                            : hours.endTime,
                      },
                    );
      ...
      >
        ...
        <DialogActions>
          ...
          <Button
            ...
            onClick={() =>
              void run(async () => {
                await agent.delete(`/practitioners/${remove!.id}`);
                setRemove(null);
              })
            }
          >
