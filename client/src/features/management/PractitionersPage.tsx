...
export default function PractitionersPage() {
    ...
    const events = useQuery({ queryKey: ['doctor-events', session.version, selected?.id], enabled: !!selected, queryFn: async ({ signal }) => (await agent.get<{ id: string; title: string; date: string }[]>(`/practitioners/${selected!.id}/activities`, { signal })).data });
    async function run(action: () => Promise<void>) { setBusy(true); setError(''); try { await action(); await Promise.all(['practitioners','working-hours','booking','activity-assignment-options','activities'].map(key => cache.invalidateQueries({ queryKey: [key] }))); } catch (e) { setError(errorText(e)); } finally { setBusy(false); } }
    ...
    return <Box sx={{ display: 'grid', gap: 2 }}><Typography variant="h4">Kezelők – orvosok és gyógytornászok</Typography>
        ...
        {!admin && <Alert severity="info">A kezelők adatait megtekintheted, a munkaidejüket módosíthatod. A kezelőprofilokat és a foglalás engedélyezését az admin kezeli.</Alert>}
        ...
        <Dialog open={!!form} onClose={() => { if (!busy) setForm(null); }} fullWidth><DialogTitle>{form?.id ? 'Kezelő szerkesztése' : 'Új kezelő'}</DialogTitle><DialogContent>{error && <Alert severity="error">{error}</Alert>}
            <Box component="form" id="doctor-form" sx={{ display: 'grid', gap: 2, pt: 1 }} onSubmit={e => { e.preventDefault(); void run(async () => { if (form?.id) await agent.put(`/practitioners/${form.id}`, form); else await agent.post('/practitioners', form); setForm(null); }); }}>
                ...
                <TextField select label="Felhasználói fiók" required disabled={!!form?.id} {...field('userId')}>{users.data?.filter(u => u.roles.includes('Practitioner')).map(u => <MenuItem key={u.id} value={u.id}>{u.userName}</MenuItem>)}</TextField>
