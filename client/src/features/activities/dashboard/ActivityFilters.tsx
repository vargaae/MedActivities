...
type Props = { filter: ActivityFilter; date: Date | null; category: string; onFilter: (value: ActivityFilter) => void; onDate: (value: Date | null) => void; onCategory: (value: string) => void;
    canFilterPeople: boolean; patients: ActivityPerson[]; practitioners: ActivityPerson[]; patientId: string; practitionerId: string; onPatient: (id: string) => void; onPractitioner: (id: string) => void };
export default function ActivityFilters({ filter, date, category, onFilter, onDate, onCategory, canFilterPeople, patients, practitioners, patientId, practitionerId, onPatient, onPractitioner }: Props) {
    return <Box sx={{ display: 'grid', gap: 3 }}>
        <Paper sx={{ p: 3, borderRadius: 3 }}><Typography variant="h6">Szűrők</Typography>
            ...
            {canFilterPeople && <>
                <TextField fullWidth select label="Páciens szerinti szűrés" value={patientId} onChange={e => onPatient(e.target.value)} sx={{ mt: 2 }}>
                    <MenuItem value="">Minden látható páciens</MenuItem>{patients.map(p => <MenuItem key={p.id} value={p.id}>{p.name}</MenuItem>)}
                </TextField>
                <TextField fullWidth select label="Kezelőorvos szerinti szűrés" value={practitionerId} onChange={e => onPractitioner(e.target.value)} sx={{ mt: 2 }}>
                    <MenuItem value="">Minden látható kezelőorvos</MenuItem>{practitioners.map(p => <MenuItem key={p.id} value={p.id}>{p.name}</MenuItem>)}
                </TextField>
            </>}
        </Paper>
        <Paper sx={{ p: 3, borderRadius: 3 }}><Typography variant="h6" sx={{ mb: 2 }}>Dátum kiválasztása</Typography>
            ...
            <Button onClick={() => { onFilter('all'); onCategory(''); onDate(null); onPatient(''); onPractitioner(''); }}>Összes szűrő törlése</Button>
        </Paper>
    </Box>;
}
