param([string]$ExportDirectory = (Join-Path $PSScriptRoot 'exports/20260915'))
...
try {
    ...
    if ($created -and $testDatabase -match '^MedActivities_ExportVerify_[a-f0-9]{32}$') {
        & sqlcmd -S '(localdb)\MSSQLLocalDB' -E -C -b -d master -Q "ALTER DATABASE [$testDatabase] SET SINGLE_USER WITH ROLLBACK IMMEDIATE; DROP DATABASE [$testDatabase];"
