...
public class UserManagementController(UserManager<AppUser> users, AppDbContext db, AccessService access) : ControllerBase
{
    ...
    public async Task<IActionResult> List()
    {
        ...
        foreach (var u in await users.Users.OrderBy(u => u.UserName).ToListAsync())
            result.Add(new
            {
                ...
                disabled = await users.IsLockedOutAsync(u)
            });
        return Ok(result);
    }
    [HttpPost]
    public async Task<IActionResult> Create(ManagedUserInput input)
    {
    ...
    [HttpPut("{id}")]
    public async Task<IActionResult> Edit(string id, ManagedUserInput input)
    {
        var u = await users.FindByIdAsync(id); if (u is null) return NotFound();
        var isDemoLoginAccount = DemoSessionSetup.Roles
            .Any(role => id == DemoSessionSetup.Prefix + role);

        if (id == access.UserId || isDemoLoginAccount)
            return Conflict(new
            {
                message = "A saját fiók és a négy demóbelépési alapfiók nem szerkeszthető."
            });
    ...
    .Any(role => id == DemoSessionSetup.Prefix + role);
        ...
        if (id == access.UserId || isDemoLoginAccount)
            return Conflict(new
            {
                message = "A saját fiók és a négy demóbelépési alapfiók nem törölhető."
            });
        var u = await users.FindByIdAsync(id); if (u is null) return NotFound();
